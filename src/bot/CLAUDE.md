...
| `bot/groups` | Group-chat identity, plain functions (no plugin, no derive): `isGroupChat(ctx)` / `isPrivateChat(ctx)` chat-type predicates + `isGroupAdmin(ctx, { chatId?, userId? }?)` — the `getChatMember` creator/administrator check every admin-gated group setting needs, chat/user defaulted from the ctx. Accepts real gramio ctxs AND `MenuCtx` with no cast. A miswired ctx (no `bot.api.getChatMember`) panics; an API rejection fails CLOSED (`false`) — it's a permission gate. Policy stays at the call site: `ctx.isAdmin \|\| await isGroupAdmin(ctx)`. Anonymous admins fail the check on messages (they wear the group's identity); callback taps always carry the real user. |
