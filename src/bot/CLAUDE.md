...
| `bot/access-control` | `accessControl({ session, storage, defaults?, gateGroups?, gateDms? })` — gates non-admin/non-default users; admin gets DM with `[✅ Aprobar][❌ Denegar]` on first attempt; persistent `/access` admin menu for revoke/reapprove/list. `gateGroups: true` gates ROOMS too (group/supergroup/channel adds): added by an admin/default → approved on the spot; anyone else → pending (bot stays but serves nothing) + admin DM `[✅ Approve][🚪 Leave]`; leave = deny with memory (re-added → leaves on sight); approving a room admits every member (`ctx.access.source === 'group'`); ungated pre-existing rooms surface as pending on first activity; `/access` grows a `👥 Groups` view; `onGroupRequest/onGroupApprove/onGroupDeny` hooks (send your welcome card on approve, not on add). The gates are independent: `gateDms: false` runs the room gate without the DM gate — unknown DM users pass (`source: 'open'`), an explicit deny still holds (a ban list for an open bot). Exposes `simulateAccessRequest()` for tests. |
...
| `bot/metrics` | `defineMetrics(spec, { write, funnels? })` — the flags pattern for MEASUREMENT: one declaration per metric (`counter`/`timing`, `perUser: true` unlocks uniques/repeat/retention/funnels over the shared user space), `bump`/`record` fire-and-forget at call sites, `describe()` renders panels generically. Storage contract: injected atomic day-bucket upsert into `metric (key, day, count, sum)` + `metric_user (key, user, day, count, sum)` when `user` is present. Funnels are declared pairs, validated perUser, computed as queries — not stored. Per-user rows join the forget-user wipe; keys are declared identifiers (the registry is the cardinality cap); money stays in ledgers. |
