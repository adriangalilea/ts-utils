...
| `bot/ctx` | `BotMessageCtx<S, Api>` / `BotCallbackCtx<S, Query, Api>` / `BotPaymentCtx<S, Api>` / `BotPreCheckoutCtx<Api>` — canonical structural ctx shapes that gramio's real contexts satisfy by duck-typing. `narrow<T>(ctx)` is the documented cast helper. Also home of the bot-id key namespace — `botId(ctx)`, `botStorageKey(ctx, userId)`, `botSubKey(ctx, sub)` — pure functions of `ctx.bot.info.id` (no env, no fs), which keeps every plugin that keys storage Worker-safe. Plugins parameterize with their `Session` shape + strict `Api` (per-method-typed `bot.api`) and stop redeclaring "ctx has session + from + send + …" in every handler. |
