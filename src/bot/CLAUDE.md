...
| `bot/access-control` | `accessControl({ session, storage, defaults?, gateGroups? })` — gates non-admin/non-default users; admin gets DM with `[✅ Aprobar][❌ Denegar]` on first attempt; persistent `/access` admin menu for revoke/reapprove/list. `gateGroups: true` gates ROOMS too (group/supergroup/channel adds): added by an admin/default → approved on the spot; anyone else → pending (bot stays but serves nothing) + admin DM `[✅ Approve][🚪 Leave]`; leave = deny with memory (re-added → leaves on sight); approving a room admits every member (`ctx.access.source === 'group'`); ungated pre-existing rooms surface as pending on first activity; `/access` grows a `👥 Groups` view; `onGroupRequest/onGroupApprove/onGroupDeny` hooks (send your welcome card on approve, not on add). Exposes `simulateAccessRequest()` for tests. |
