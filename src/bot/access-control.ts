...
const userSessionKey = (userId: number) => `${SESSION_KEY_PREFIX}${userId}`
...
type AccessSessionDerives = { _accessSession: AccessRecord }
...
const warnedMemoryStorage = (): Storage => {
  if (!warnedMemory) {
    warnedMemory = true
    console.warn(
      '[access-control] using inMemoryStorage — approvals will not survive restarts. ' +
...
const loadRecord = async (
  ...
  (await storage.get(userSessionKey(userId))) as AccessRecord | undefined

const saveRecord = (storage: Storage, userId: number, rec: AccessRecord) =>
  storage.set(userSessionKey(userId), rec)

// ─── plugin ────────────────────────────────────────────────────────

export const accessControl = (opts: AccessControlOptions = {}) => {
  const storage: Storage = opts.storage ?? warnedMemoryStorage()
    ...
    new Plugin<{}, AcDerives>('@adriangalilea/utils/bot/access-control', {
      ...
      .extend(
        session({
          ...
          initial: (): AccessRecord => ({ status: 'unknown' }),
        }),
      )
...
const listView = async (
  ...
  const records = await Promise.all(
    shownIds.map(async (id) => ({ id, rec: await loadRecord(storage, id) })),
  )
