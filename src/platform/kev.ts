...
import { findMonorepoRoot, findProjectRoot } from "./project.js";
...
export class KevOps {
	...
	initializeSmartDefaults(): void {
		...
		if (this.debug) {
			if (!monorepoRoot && !projectRoot) {
				console.log(
					"KEV: No project or monorepo root found, using standard sources:",
					this.sources,
				);
			} else {
		...
		for (const source of this.sources) {
			...
			if (val !== "") {
				...
				const absoluteSource =
					source !== "os" && source !== "default" && source !== "set"
	...
	clear(...patterns: string[]): void {
		...
		for (const pattern of patterns) {
			if (pattern.includes(":")) {
				panic(
					"Clear() with namespace is dangerous! Use clearUnsafe() if you really need this.",
				);
			}
		}
		...
		for (const pattern of patterns) {
			...
			switch (namespace) {
				case "os": {
					if (keyPattern === "*") {
						panic(
							'clearUnsafe("os:*") would destroy system! This is never allowed.',
						);
					}
