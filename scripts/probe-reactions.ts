...
if (!token || !chatId || !messageId) {
	console.error(
		"usage: BOT_TOKEN=… bun scripts/probe-reactions.ts <chat_id> <message_id>",
	);
	process.exit(2);
}
...
console.log(
	`probed ${CANDIDATES.length} candidates against ${chatId}/${messageId}`,
);
console.log(`accepted ${accepted.length}: ${accepted.join(" ")}`);
console.log(
	`refused ${refused.length}: ${refused.map((r) => r.emoji).join(" ")}`,
);
console.log(
...
console.log(
	`export const TELEGRAM_REACTIONS = ${JSON.stringify(accepted)} as const`,
);
