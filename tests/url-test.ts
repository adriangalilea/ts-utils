...
assert.equal(
	cleanUrl("https://example.com/a?fbclid=abc123"),
	"https://example.com/a",
);
// Vendored global list: analytics + email-marketing ids
assert.equal(
	cleanUrl(
		"https://example.com/a?gclid=1&mc_eid=2&_hsenc=3&mkt_tok=4&pk_campaign=5&keep=yes",
	),
...
assert.equal(
	cleanUrl(
		"https://example.com/a?msclkid=1&twclid=2&ttclid=3&srsltid=4&_ga=5&keep=yes",
	),
	"https://example.com/a?keep=yes",
);
// Case-insensitive param matching
assert.equal(
	cleanUrl("https://example.com/a?UTM_Source=x&b=1"),
...
assert.equal(
	cleanUrl("https://example.com/a?t=1&t=2"),
	"https://example.com/a?t=1&t=2",
);

// ── cleanUrl: per-host rules strip there, never elsewhere ───────────────────

assert.equal(
	cleanUrl("https://youtu.be/dQw4w9WgXcQ?si=share_junk"),
	"https://youtu.be/dQw4w9WgXcQ",
);
assert.equal(
	cleanUrl(
		"https://www.youtube.com/watch?v=dQw4w9WgXcQ&si=x&feature=share&ab_channel=Foo&pp=xyz",
	),
	"https://www.youtube.com/watch?v=dQw4w9WgXcQ",
);
// Bare host (no www.) — upstream's matcher misses it, the overlay covers it
assert.equal(
	cleanUrl("https://youtube.com/watch?v=abc&feature=share"),
	"https://youtube.com/watch?v=abc",
);
// Subdomains match
assert.equal(
	cleanUrl("https://music.youtube.com/watch?v=abc&si=x"),
...
assert.equal(
	cleanUrl("https://open.spotify.com/track/xyz?si=abc"),
	"https://open.spotify.com/track/xyz",
);
assert.equal(
	cleanUrl("https://medium.com/@a/post-1?source=rss"),
	"https://medium.com/@a/post-1",
);
// Medium's `sk` unlocks friend links — must survive
assert.equal(
	cleanUrl("https://medium.com/@a/post-1?sk=secret"),
	"https://medium.com/@a/post-1?sk=secret",
);
// x.com (overlay) and twitter.com (vendored + overlay `t`)
assert.equal(
	cleanUrl("https://x.com/user/status/1?s=20&t=tok"),
	"https://x.com/user/status/1",
);
assert.equal(
	cleanUrl("https://twitter.com/user/status/1?s=20&t=tok"),
...
assert.equal(
	cleanUrl("mailto:a@b.com?subject=utm_source"),
	"mailto:a@b.com?subject=utm_source",
);
assert.equal(
	cleanUrl("ftp://example.com/file?utm_source=x"),
...
assert.equal(
	urlKey("https://example.com/post?utm_source=a&id=7&utm_medium=b"),
	KEY,
); // tracking
...
assert.equal(
	urlKey("https://example.com/a?b=2&a=1"),
	urlKey("https://example.com/a?a=1&b=2"),
);
// …but different values are different resources
assert.notEqual(
	urlKey("https://example.com/a?page=1"),
	urlKey("https://example.com/a?page=2"),
);
// Path case is identity (many servers are case-sensitive)
assert.notEqual(
	urlKey("https://example.com/About"),
	urlKey("https://example.com/about"),
);
// Non-default port is identity
assert.notEqual(
	urlKey("https://example.com:8443/a"),
...
assert.equal(
	urlKey(
		"https://www.theverge.com/2026/1/1/some-story?utm_source=twitter&utm_campaign=social",
	),
...
assert.equal(
	cleanUrl("https://example.com/a?session=x&id=1", { strip: ["session"] }),
	"https://example.com/a?id=1",
);
assert.equal(
	urlKey("https://example.com/a?Session=x&id=1", { strip: ["session"] }),
	"example.com/a?id=1",
);
...
{
	const text =
		"check https://a.com/x?utm_source=t and also example.com/y thanks";
	...
	assert.equal(
		text.slice(found[0].start, found[0].end),
...
assert.deepEqual(
	urlsIn("https://a.com/x and example.com/y", { requireScheme: true }).map(
		(u) => u.href,
	),
	["https://a.com/x"],
);
...
{
	const found = urlsIn(
		"https://youtu.be/dQw4w9WgXcQ and https://www.youtube.com/watch?v=dQw4w9WgXcQ&feature=share",
	);
...
assert.equal(
	urlOf("https://example.com/a?utm_source=x")?.href,
	"https://example.com/a",
);
...
assert.equal(
	youtubeTimestampUrl(YT, 61),
	`https://www.youtube.com/watch?v=${YT}&t=61s`,
);
assert.equal(
	youtubeTimestampUrl("https://example.com/a", 30),
	"https://example.com/a",
); // not a video
assert.equal(
	youtubeTimestampUrl(`https://youtu.be/${YT}`, Number.NaN),
	`https://youtu.be/${YT}?t=0s`,
);

// urlKey collapses every spelling of one video to one identity
assert.equal(urlKey(`https://youtu.be/${YT}?si=x`), `youtube:${YT}`);
assert.equal(
	urlKey(`https://music.youtube.com/watch?v=${YT}`),
	`youtube:${YT}`,
);
assert.equal(urlKey(`https://www.youtube.com/shorts/${YT}`), `youtube:${YT}`);
// …but a non-video youtube page keys generically
assert.equal(
	urlKey("https://www.youtube.com/@somechannel"),
...
console.log(
	"✓ url-test: urlsIn/urlOf/urlKey/cleanUrl/youtube identity/host helpers hold",
);
