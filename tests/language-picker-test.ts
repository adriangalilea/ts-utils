...
await ok(
	"picker item: 2-up rows, active code wears primary, pick gets the code",
	async () => {
		const picked: string[] = [];
		...
		const entries = (item as { submenu: Array<Record<string, unknown>> })
			.submenu;
		assert.deepEqual(
		...
		const styles = await Promise.all(
			entries.map((e) =>
				(e.style as (c: MenuCtx) => Promise<string | undefined>)(ctx),
			),
		);
		assert.deepEqual(styles, [undefined, "primary", undefined]);
		assert.equal(
			await (entries[1].action as (c: MenuCtx) => unknown)(ctx),
			"✓ es",
		);
...
await ok(
	"addLanguageRows: appends 2-up after lead rows, active styled, trailing rows chain",
	() => {
